...
namespace human {
    ...
    int playCount = 6;
    getRd rd(0, all.size() - 1);
    string hidden = all[rd(random)];
    ...
    string getPredict() {
        for (int i = 1; const auto& [inp, status] : predictions) {
            cout << "----------------Attempt " << i << "----------------\n";
    ...
    prediction::prediction(const string& _inp) {
        inp = _inp;
        if (!binary_search(all.begin(), all.end(), inp)) status = "Invalid input";
        else status = compare(inp, hidden);
    }

    int check() {
        if (predictions.empty() || predictions.back().status == "Invalid feedback") return 0;
        if (predictions.back().status == "BBBB") return 2;
        return 1;
    }
}
...
namespace bot {
    ...
    int playCount = 6;
    getRd rd(0, all.size() - 1);
    string botNextPredict = all[rd(random)];
    void play();
    string getFeedback();
    int check();
    ...
    prediction::prediction(const string &_feedback) {
        ...
        for (const string& predict : all) {
            if (compare(botNextPredict, predict) == feedback) newAll.push_back(predict);
        }
        if (newAll.empty()) {
